...
function send_moble($moble, $content)
{
	...
	$_SESSION['time'] = time();
	$name = C('moble_url');//网站的名称
	//$content = "海通网络".$content;
	...
	//$url = 'http://utf8.sms.webchinese.cn/?Uid=17755193851&Key=zlw665588&smsMob=' . $moble . '&smsText=' . $content;
...
function chkAuthCache()
{
	...
	debug('A');
	$md2 = md5(md5($key1) + md5('ecshecom') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
	...
	debug('D');
	$times--;
	$md = md5(md5($key1) + md5('ecshecom') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
...
function ecshecom_getCoreConfig(){
	$file_path = DATABASE_PATH . '/e'.'c'.'s'.'h'.'e'.'c'.'o'.'m'.'.'.'j'.'s'.'o'.'n';
	if (file_exists($file_path)) {
		$ecshecom_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$ecshecom_CoreConfig = json_decode($ecshecom_CoreConfig, true);
		$SystemQQ = base64_decode("NTAyMTgyMjk5");
		if($ecshecom_CoreConfig['authQQ']==$SystemQQ){
			return $ecshecom_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/ecshecom/log.html";
