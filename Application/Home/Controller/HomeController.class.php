...
class HomeController extends \Think\Controller
{
	protected function _initialize()
	{
		...
		/* if(WAP_URL !=""){
			...
			if(preg_match('/(iphone|android|Windows\sPhone)/i', $ua)){
				...
				if (isset($_GET['invit'])) {
					...
					if ($user['id']) {
						$ecshecom_redirect = WAP_URL."/Login/register/invit/".$user['id'];
					}else{
						$ecshecom_redirect = WAP_URL;
					}
				}else{
					$ecshecom_redirect = WAP_URL;
				}
		...
		//20170511 QQ502182299 增加获取币类型函数
		
		//C('coin_menu_ecshecom',array('CNY','BTC','ETH'));
