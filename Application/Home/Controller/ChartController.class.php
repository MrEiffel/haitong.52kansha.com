...
class ChartController extends HomeController
{
	public function getJsonData($market = NULL, $ajax = 'json')
	{
		if ($market) {
			...
			if(!$ecshecom_getCoreConfig){
				...
				$ecshecom_putong = $ecshecom_getCoreConfig['ecshecom_userTradeDetailNum'];
				$ecshecom_teshu = $ecshecom_getCoreConfig['ecshecom_specialUserTradeDetailNum'];
			}
			
			$limit = $ecshecom_putong;
			if(userid()){
				$usertype = M('User')->where(array($id => $userid))->getField('usertype');
				if($usertype ==1){
					$limit = $ecshecom_teshu;
				}else{
					$limit = $ecshecom_putong;
				}
			}
	...
	protected function getTradeBuy($market,$limit)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from ecshecom_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit '.$limit);
		$data = '';
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from ecshecom_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
	...
	public function getMarketNewKQQ502182299Json()
	{
		// TODO: SEPARATE
		$input = I('get.');
		$market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
