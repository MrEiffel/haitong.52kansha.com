...
class ShopController extends HomeController
{
	...
	public function view($id)
	{
		...
		if (!$Shop) {
			...
			$Shop['ecshecom_awardcoin'] =  C('coin')[$Shop['ecshecom_awardcoin']]['title']; 
...
/* 
		...
		$ecshecom_awardcoinnum  = $Shop['ecshecom_awardcoinnum'];
		if($ecshecom_awardcoinnum>0){
			$ecshecom_awardcoinnum = $ecshecom_awardcoinnum * $num;
		}
		$ecshecom_awardcointype = $Shop['ecshecom_awardcoin'];
		...
		$mo->execute('lock tables ecshecom_user_coin write,ecshecom_shop write,ecshecom_shop_log write');
		$rs = array();
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => userid()))->setDec($type, $xuyao);
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => userid()))->setInc($ecshecom_awardcointype,$ecshecom_awardcoinnum);
		$rs[] = $mo->table('ecshecom_shop')->where(array('id' => $Shop['id']))->save(array(
			'deal' => array('exp', 'deal+' . $num),
			'num'  => array('exp', 'num-' . $num)
			));

		if ($Shop['num'] - $num <= 0) {
			$rs[] = $mo->table('ecshecom_shop')->where(array('id' => $Shop['id']))->save(array('status' => 0));
		}

		$rs[] = $mo->table('ecshecom_shop_log')->add(array('userid' => userid(), 'shopid' => $Shop['id'], 'price' => $Shop['price'], 'coinname' => $type, 'xuyao' => $xuyao, 'num' => $num, 'mum' => $mum, 'addr' => $my_goods['truename'] . '|' . $my_goods['moble'] . '|' . $my_goods['addr'], 'addtime' => time(), 'status' => 0));
