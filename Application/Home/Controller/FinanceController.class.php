...
/* 		foreach ($CoinList as $k => $v) {
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables ecshecom_mytx write , ecshecom_user_coin write ,ecshecom_finance write');
		$rs = array();
		...
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
		$rs[] = $finance_nameid = $mo->table('ecshecom_mytx')->add(array('userid' => userid(), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function upmyzc($coin, $num, $addr, $paypassword, $moble_verify)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			...
			$rs[] = $mo->table('ecshecom_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('ecshecom_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$mo->execute('lock tables  ecshecom_user_coin write  , ecshecom_myzc write ,ecshecom_myzr write, ecshecom_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('ecshecom_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $aid = $mo->table('ecshecom_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));
	...
	public function upmyzr($coin,$ecshecom_dzbz,$num, $paypassword, $moble_verify)
	{
		if (!userid()) {
			$this->error('您没有登录请先登录！');
		}
		...
		if ($Coin['type'] == 'rgb') {
			M('myzr')->add(array(
			    'userid' => userid(),
                'username' => $ecshecom_dzbz,
                'txid' => $ecshecom_zrcoinaddress,
                'coinname' => $coin,
                'num' => $num,
                'mum' => 0,
                'addtime' => time(),
    ...
    public function upmycustomcoinzr($coin, $code_verify)
    {
        ...
        if ($result) {
            // 前台“确认转入”按钮，限制半小时按一次
            session('myzr_limit_' . $coin, $time + 1800);
            $this->success('转入申请成功,等待客服处理！');
        } else {
            $this->error("钱包币不允许该操作!");
        }
