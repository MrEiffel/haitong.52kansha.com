...
class AjaxController extends HomeController
{
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		if (!$data) {
			...
			foreach($ecshecom_getCoreConfig['ecshecom_indexcat'] as $k=>$v){
				$data[$k][title] = $v;
			}
			...
			S('ecshecom_getTopCoinMenu', $data);
		}
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if(!$ecshecom_getCoreConfig){
			...
			$ecshecom_putong = $ecshecom_getCoreConfig['ecshecom_userTradeNum'];
			$ecshecom_teshu = $ecshecom_getCoreConfig['ecshecom_specialUserTradeNum'];
		}
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		$result = M()->query('select id,price,num,deal,mum,type,fee,status,addtime from ecshecom_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
