		...
		if (!userid()) {
			...
			if($data['awardStatus']==3){
				...
				if($prize_id>0 && $prize_status==0){
					...
					$mo->execute('lock tables ecshecom_user_award write,ecshecom_user write');
					$rs[] = $mo->table('ecshecom_user_award')->add($awardInfo);
					$rs[] = $mo->table('ecshecom_user')->save($uinfo);
					$flag = true;
					if ($rs) {
						if ($flag) {
							$mo->execute('commit');
					...
					$mo = M();
					$mo->execute('set autocommit=0');
					$mo->execute('lock tables ecshecom_user write');
					$rs = $mo->table('ecshecom_user')->save($uinfo);
					$flag = true;
					if ($rs) {
						if ($flag) {
