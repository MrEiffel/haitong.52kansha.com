...
class UserController extends CommonController
{
	...
	public function upmytx($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables ecshecom_mytx write , ecshecom_user_coin  write ');
		$rs = array();
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setDec('cny', $num);
		$rs[] = $mo->table('ecshecom_mytx')->add(array('userid' => $uid, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function upmyzc($coin_id, $moble_verify, $num, $paypassword, $addr)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);

			if ($fee) {
				if ($mo->table('ecshecom_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
					$rs[] = $mo->table('ecshecom_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
				else {
					$rs[] = $mo->table('ecshecom_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
			}

			$rs[] = $mo->table('ecshecom_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			$rs[] = $mo->table('ecshecom_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));

			if ($fee_user) {
				$rs[] = $mo->table('ecshecom_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $Coin['zc_user'] . time()), 'num' => $num, 'fee' => $fee, 'type' => 1, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			}
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('ecshecom_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$mo->execute('lock tables  ecshecom_user_coin write  , ecshecom_myzc write ,ecshecom_myzr write, ecshecom_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setDec($coin, $num);
				debug(array('res' => $r, 'lastsql' => $mo->table('ecshecom_user_coin')->getLastSql()), '更新用户人民币');
				$rs[] = $aid = $mo->table('ecshecom_myzc')->add(array('userid' => $uid, 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));

				if ($fee && $auto_status) {
					$rs[] = $mo->table('ecshecom_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'type' => 2, 'addtime' => time(), 'status' => 1));

					if ($mo->table('ecshecom_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
						$rs[] = $r = $mo->table('ecshecom_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
						debug(array('res' => $r, 'lastsql' => $mo->table('ecshecom_user_coin')->getLastSql()), '新增费用');
					}
					else {
						$rs[] = $r = $mo->table('ecshecom_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					}
				}

				debug(array('res' => $r, 'lastsql' => $mo->table('ecshecom_myzc')->getLastSql()), '转出记录');
	...
	public function cancel_mywt($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables ecshecom_user_coin write  , ecshecom_trade write ');
		$rs = array();
		...
		if ($trade['type'] == 1) {
			...
			if (!$you_sell) {
				...
				if (0 < $you_user_sell[$xnb . 'd']) {
					$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $trade['userid']))->setField($xnb . 'd', 0);
				}
			}
		}
		else {
