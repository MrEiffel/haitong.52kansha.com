...
class TradeController extends CommonController
{
	...
	public function getTradedata($marketid = NULL)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from ecshecom_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 5;');
		$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from ecshecom_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 5;'));
	...
	public function upTrade($paypassword = NULL, $marketid = NULL, $price, $num, $type)
	{
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from ecshecom_trade where userid=' . $uid . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables ecshecom_trade write ,ecshecom_user_coin write');
		$rs = array();

		if ($type == 1) {
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setDec($rmb, $mum);
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setInc($rmb . 'd', $mum);
			$rs[] = $mo->table('ecshecom_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
			$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $uid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('ecshecom_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
	...
	public function matchingTrade($market = NULL)
	{
		...
		$mo = M();
		$mo->table('ecshecom_trade')->where('num<deal')->setField('status', 1);
		...
		for (; true; ) {
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables ecshecom_trade write ,ecshecom_trade_log write ,ecshecom_user write,ecshecom_user_coin write,ecshecom_invit write');
				...
				$rs[] = $mo->table('ecshecom_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
