...
class ShopController extends AdminController
{
	...
	public function edit($id = NULL)
	{
		...
		if (APP_DEMO) {
			...
			$builder->keyText('ecshecom_awardcoinnum', '奖励币种数量', '整数,留空或者填写0 不奖励');
			$builder->keySelect('ecshecom_awardcoin', '奖励币种', '奖励币种', $coin_list);
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables ecshecom_user_coin write,ecshecom_shop_log write');
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('ecshecom_shop_log')->where(array('id' => $id))->save(array('status' => 2));
