...
class FinanceController extends AdminController
{
	...
	public function myzrQueren()
	{
		...
		$mo->execute('lock tables ecshecom_user_coin write,ecshecom_myzr write,ecshecom_finance write,ecshecom_invit write,ecshecom_user write');
		$rs = array();

		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $myzr['userid']))->setInc($myzr['coinname'], $myzr['num']);
		$rs[] = $mo->table('ecshecom_myzr')->where(array('id' => $myzr['id']))->save(array('status' => 1, 'mum' => $myzr['num'], 'endtime' => time()));
	...
	public function myczQueren()
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables ecshecom_user_coin write,ecshecom_mycz write,ecshecom_finance write,ecshecom_invit write,ecshecom_user write');
		$rs = array();
		$finance = $mo->table('ecshecom_finance')->where(array('userid' => $mycz['userid']))->order('id desc')->find();
		$finance_num_user_coin = $mo->table('ecshecom_user_coin')->where(array('userid' => $mycz['userid']))->find();
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
		$rs[] = $mo->table('ecshecom_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 2, 'mum' => $mycz['num'], 'endtime' => time()));
		$finance_mum_user_coin = $mo->table('ecshecom_user_coin')->where(array('userid' => $mycz['userid']))->find();
		$finance_hash = md5($mycz['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mycz['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.ecshecom.com');
		...
		$cur_user_info = $mo->table('ecshecom_user')->where(array('id' => $mycz['userid']))->find();
		...
		if($cur_user_info['invit_1']&&$cur_user_info['invit_1']>0&&1==2){
			...
			if ($invit_1_jiner) {
				//处理前信息
				$finance_1 = $mo->table('ecshecom_finance')->where(array('userid' => $cur_user_info['invit_1']))->order('id desc')->find();
		        $finance_num_user_coin_1 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();
				
				//开始处理
				$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->setInc('cny',$invit_1_jiner);
				$rs[] = $mo->table('ecshecom_invit')->add(array('userid' => $cur_user_info['invit_1'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '一代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_1_jiner, 'addtime' => time(), 'status' => 1));
				
				//处理后
				$finance_mum_user_coin_1 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();
				$finance_hash_1 = md5($cur_user_info['invit_1'].$finance_num_user_coin_1['cny'] . $finance_num_user_coin_1['cnyd'] . $invit_1_jiner . $finance_mum_user_coin_1['cny'] . $finance_mum_user_coin_1['cnyd'] . MSCODE . 'auth.ecshecom.com');
		...
		if($cur_user_info['invit_2']&&$cur_user_info['invit_2']>0&&1==2){
			...
			if ($invit_2_jiner) {
				...
				//处理前信息
				$finance_2 = $mo->table('ecshecom_finance')->where(array('userid' => $cur_user_info['invit_2']))->order('id desc')->find();
		        $finance_num_user_coin_2 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();
				
				//开始处理
				
				$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->setInc('cny',$invit_2_jiner);
				$rs[] = $mo->table('ecshecom_invit')->add(array('userid' => $cur_user_info['invit_2'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '二代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_2_jiner, 'addtime' => time(), 'status' => 1));
			
				//处理后
				$finance_mum_user_coin_2 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();
				$finance_hash_2 = md5($cur_user_info['invit_2'].$finance_num_user_coin_2['cny'] . $finance_num_user_coin_2['cnyd'] . $invit_2_jiner . $finance_mum_user_coin_2['cny'] . $finance_mum_user_coin_2['cnyd'] . MSCODE . 'auth.ecshecom.com');
		...
		if($cur_user_info['invit_3']&&$cur_user_info['invit_3']>0&&1==2){
			...
			if ($invit_3_jiner) {
				...
				//处理前信息
				$finance_3 = $mo->table('ecshecom_finance')->where(array('userid' => $cur_user_info['invit_3']))->order('id desc')->find();
		        $finance_num_user_coin_3 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();
				
				//开始处理

				$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->setInc('cny',$invit_3_jiner);
				$rs[] = $mo->table('ecshecom_invit')->add(array('userid' => $cur_user_info['invit_3'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '三代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_3_jiner, 'addtime' => time(), 'status' => 1));
			
				//处理后
				$finance_mum_user_coin_3 = $mo->table('ecshecom_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();
				$finance_hash_3 = md5($cur_user_info['invit_3'].$finance_num_user_coin_3['cny'] . $finance_num_user_coin_3['cnyd'] . $invit_3_jiner . $finance_mum_user_coin_3['cny'] . $finance_mum_user_coin_3['cnyd'] . MSCODE . 'auth.ecshecom.com');
	...
	public function mytxChexiao()
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables ecshecom_user_coin write,ecshecom_mytx write,ecshecom_finance write');
		$rs = array();
		$finance = $mo->table('ecshecom_finance')->where(array('userid' => $mytx['userid']))->order('id desc')->find();
		$finance_num_user_coin = $mo->table('ecshecom_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$rs[] = $mo->table('ecshecom_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
		$rs[] = $mo->table('ecshecom_mytx')->where(array('id' => $mytx['id']))->setField('status', 2);
		$finance_mum_user_coin = $mo->table('ecshecom_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$finance_hash = md5($mytx['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mytx['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.ecshecom.com');
		$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
	...
	public function ecshecom_financeExcel()
	{
		if (IS_POST) {
			$id = implode(',', $_POST['id']);
		}
	...
	public function ecshecom_financeAllExcel()
	{
