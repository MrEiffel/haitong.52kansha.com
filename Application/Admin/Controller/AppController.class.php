...
class AppController extends AdminController
{
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('ecshecom_user ON ecshecom_user.id = ecshecom_app_vipuser.uid')->join('ecshecom_app_vip ON ecshecom_app_vip.id = ecshecom_app_vipuser.vip_id')->field('ecshecom_user.username,ecshecom_app_vipuser.*,ecshecom_app_vip.name as vip_name,ecshecom_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('ecshecom_user ON ecshecom_user.id = ecshecom_app_vipuser.uid')->join('ecshecom_app_vip ON ecshecom_app_vip.id = ecshecom_app_vipuser.vip_id')->field('ecshecom_user.username,ecshecom_app_vipuser.*,ecshecom_app_vip.name as vip_name,ecshecom_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
