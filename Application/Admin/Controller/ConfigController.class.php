	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				...
				$rea = M()->execute('ALTER TABLE  `ecshecom_user_coin` ADD  `' . $_POST['name'] . '` DECIMAL(20,8) UNSIGNED NOT NULL');
				$reb = M()->execute('ALTER TABLE  `ecshecom_user_coin` ADD  `' . $_POST['name'] . 'd` DECIMAL(20,8) UNSIGNED NOT NULL ');
