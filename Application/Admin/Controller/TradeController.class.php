...
class TradeController extends AdminController
{
	public function index($field = NULL, $name = NULL, $market = NULL, $status = NULL,$type=0)
	{
		...
		$ecshecom_getSum = M('Trade')->where($where)->sum('mum');
		...
		$this->assign('ecshecom_count', $count);
		$this->assign('ecshecom_getSum', $ecshecom_getSum);
	...
	public function log($field = NULL, $name = NULL, $market = NULL,$type=NULL)
	{
		...
		$ecshecom_getSum = M('TradeLog')->where($where)->sum('mum');
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		foreach($list as $k=>$v){
			...
			$list[$k]['tradetimeecshecom'] = $begintradeqq_3479015851."-".$endtradeqq_3479015851;
		}
	...
	public function marketEdit($id = NULL)
	{
		...
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$market_ecshecom = M('Market')->where(array('id' => $id))->find();
				$this->data = $market_ecshecom;

				if($market_ecshecom['begintrade']){
					$beginshi = explode(":",$market_ecshecom['begintrade'])[0];
					$beginfen = explode(":",$market_ecshecom['begintrade'])[1];
				}else{
					$beginshi = "00";
					$beginfen = "00";
				}
				
				if($market_ecshecom['endtrade']){
					$endshi = explode(":",$market_ecshecom['endtrade'])[0];
					$endfen = explode(":",$market_ecshecom['endtrade'])[1];
				}else{
					$endshi = "23";
					$endfen = "59";
				}
			...
			$this->assign('ecshecom_getCoreConfig',$ecshecom_getCoreConfig['ecshecom_indexcat']);
